...
Suggested Citation: “Schwab U.S. Dividend Equity ETF (SCHD).” *Yahoo
Finance*, <https://finance.yahoo.com/quote/SCHD/>. Accessed 3 Sept.
2025.
...
#' Plot Gold (Front Month) Adjusted by PPI (Ratio Series)
#'
#' Plots the series \code{GC_F.Close__by__PPIACO.Value} over a specified date range,
#' with a configurable lower y-axis limit. The upper y-limit is computed from the
#' data via \code{round_up_to(max(series), 1)}. This function expects the project
#' helpers \code{require_columns()}, \code{plotSingle()}, \code{getPlotTitle()},
#' and \code{getPlotYLabel()} as well as the data frames \code{df.data},
#' \code{df.symbols}, and \code{dfRecession} to be available in the environment.
#'
#' @param dt.start Date. Start date for the x-axis range. Default \code{as.Date("2000-01-01")}.
#' @param ylim.lower numeric. Lower bound for the y-axis. Default \code{1.5}.
#'
#' @return A \pkg{ggplot2} object (returned invisibly). The plot is also printed.
#' @examples
#' \dontrun{
#'   p <- plot_gc_over_ppi(dt.start = as.Date("2005-01-01"), ylim.lower = 2)
#'   # ggplot2::ggsave("gc_over_ppi.svg", p, width = 9, height = 5)
#' }
#' @export
plot_gc_over_ppi <- function(dt.start = as.Date("2000-01-01"),
                             ylim.lower = 1.5) {

  lst_syms <- c("GC_F.Close__by__PPIACO.Value")
  ...
  if (!is.finite(y_max)) y_max <- ylim.lower + 1
...
plot_gc_over_ppi(dt.start = as.Date("2000-01-01"),
                             ylim.lower = 1.5)
...
plot_gc_over_ppi(dt.start = seq(Sys.Date(), by = "-1 year", length.out = 2)[2],
                             ylim.lower = 9.5)
