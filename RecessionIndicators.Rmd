...
The fund invests in financial instruments that ProShare Advisors believes, in
combination, should track the performance of the index. The index is designed to
measure the performance of companies in the S&P 500 Index that have consistently
increased dividends each year for at least 25 years. Under normal circumstances,
it will invest at least 80% of its total assets in components of the index or in
instruments with similar economic characteristics.

Suggested Citation:

“ProShares S\&P 500 Dividend Aristocrats ETF (NOBL) Company Profile & Facts.” *Yahoo Finance*, [https://finance.yahoo.com/quote/NOBL/profile/](https://finance.yahoo.com/quote/NOBL/profile/). Accessed 3 Sept. 2025.
...
To pursue its goal, the fund generally invests in stocks that are included in
the index. The index is designed to measure the performance of high dividend
yielding stocks issued by U.S. companies that have a record of consistently
paying dividends, selected for fundamental strength relative to their peers,
based on financial ratios. The fund will invest at least 90% of its net assets
in these stocks.

Suggested Citation:
“Schwab U.S. Dividend Equity ETF (SCHD).” *Yahoo Finance*, [https://finance.yahoo.com/quote/SCHD/](https://finance.yahoo.com/quote/SCHD/). Accessed 3 Sept. 2025.
...
if (require_columns(df.data, lst_sym)) {
  ...
  my.plot <-
    plotSingle(
      ...
      getPlotYLabel(df.symbols, lst_sym[[1]]),
      c(as.Date('2014-01-01'), Sys.Date()),
      ylim =  c(10, round_up_to(max(df.data[[lst_sym[[1]]]]),1)),
      b.legend = TRUE,
      b.percentile = FALSE,
      b.long.legend = TRUE
    )
...
This plot shows the year-over-year  growth of the NOBL and SCHD funds to see if
...
This is a highly cyclical sector so it pays to watch for movements in this this
sector.
...
“Energy Select Sector SPDR Fund (XLE) Company Profile & Facts.” Yahoo Finance, https://finance.yahoo.com/quote/XLE/profile/
...
if (require_columns(df.data, lst_sym)) {
  ...
  my.plot <-
    plotSingle(
      ...
      ylim =  c(75, round_up_to(max(df.data[[lst_sym[[1]]]]),1)),
      b.legend = TRUE,
      b.percentile = FALSE,
      b.long.legend = TRUE
    )
...
that comprise the underlying index. The underlying index is composed of all of
the components of the S&P 500® Energy Index, an index that contains the common
stocks of all companies included in the S&P 500® Index that are classified as
members of the energy sector, as defined according to the Global Industry
Classification Standard ("GICS").

Suggested Citation:

“Invesco S&P 500 Equal Weight Growth ETF (RSPG) Company Profile & Facts.” Yahoo Finance, https://finance.yahoo.com/quote/RSPG/profile/
. Accessed 8 Sept. 2025.
...
This plot shows the year-over-year  growth of the XLE and RSPG funds to see if
there could be a buying opportunity.
...
This plot shows three smooth lines tracking employment levels across cycles,
with downturns/recoveries highlighted by the recession shading. The ADP line
often moves similarly to the BLS series but can lead slightly month-to-month;
the PAYEMS line sits above the private series because it includes government
jobs.

Payrolls roll over ahead of or early in recessions and re-accelerate during
recoveries. The shaded bands, indicating recessions, let you see that
relationship.

Comparing private (NPPTTL) to total (PAYEMS) highlights whether the public
sector is cushioning weakness (or amplifying strength).
...
This plot shows three tightly related growth-rate curves. ADP YoY often turns a
bit earlier; PAYEMS YoY is the benchmark; private YoY isolates underlying
business hiring (government swings can make PAYEMS look sturdier or weaker than
the private economy).

Inflections in YoY often precede (or coincide with) NBER recession starts and
equity draw downs.

Payroll growth ≈ breadth of labor demand; sustained deceleration usually
foreshadows softer PCE and revenue growth.

Weakening YoY increases odds of easing; re-acceleration keeps policy tighter for
longer—key for duration, curve, and equity style.
...
Suggested by Charlie, it can be helpful to look at the relationship between
...
```
What has gold done over the last year?
...
sym_plot <- "GC_F.Close__by__PPIACO.Value"
ylim <- c(0, 12)
...
plotSingle(
  ...
  datax = "date",
  datay = sym_plot,
  titlelabel =  getPlotTitle(df.symbols, sym_plot),
  xlabel = "Date",
  ylabel = getPlotYLabel(df.symbols, sym_plot),
  ylim,
...
```{r eia.total.Oilgasdrilling, echo=FALSE, , fig.width = 10, fig.asp = .62 }

lst_syms <- c("TOTAL_NGNRPUS_M",
              "TOTAL_OGNRPUS_M",
              "TOTAL_NGNRPUS_M")
...
Look at GDP divided by CPI per person. It flattens and even dips a little prior
to a recession. Might be worth looking at the derivative of this series.
