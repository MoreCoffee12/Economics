...
if(bRefresh) {
  ...
  for (idx in 1:nrow(df.symbols)) {
    ...
    str.safe <- str_replace_all(df.symbols[idx, "string.symbol"], "[^[:alnum:]]", "")
    print(str.safe)
...
if(bRefresh){
  ...
  df.silverblatt <- data.frame(read_excel('sp-500-eps-est.xlsx', range='QUARTERLY DATA!A7:I140'))
...
```{r rsalesderplot.yoy, echo=FALSE, fig.width = 5, fig.asp = .42}
datay <- "RSALESAGG_YoY" 
ylim <- c(-12.5, 12.5)
dt.start <- as.Date('1968-01-01')
myPlot <-
