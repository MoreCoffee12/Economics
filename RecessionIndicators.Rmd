...
“ProShares S&P 500 Dividend Aristocrats ETF (NOBL) Company Profile &
Facts.” *Yahoo Finance*,
...
“Schwab U.S. Dividend Equity ETF (SCHD).” *Yahoo Finance*,
...
companies by the GICS®, including securities of companies from the
...
“Energy Select Sector SPDR Fund (XLE) Company Profile & Facts.” Yahoo
...
composed of all of the components of the S&P 500® Energy Index, an index
that contains the common stocks of all companies included in the S&P
500® Index that are classified as members of the energy sector, as
defined according to the Global Industry Classification Standard
("GICS").

RSPG stands at
    `r paste("$",sprintf("%3.2f", tail(df.data$RSPG.Close,1)),sep="")`
    (last updated on `r index(tail(RSPG,1))`). 
    
The 50-day MVA stands at `r paste("$",sprintf("%3.2f", tail(df.data$RSPG.Close__mva050,1)), sep="")`. 
    
The 200-day MVA stands at `r paste("$",sprintf("%3.2f", tail(df.data$RSPG.Close__mva200,1)), sep="")`

Suggested Citation:

“Invesco S&P 500 Equal Weight Growth ETF (RSPG) Company Profile &
Facts.” Yahoo Finance, <https://finance.yahoo.com/quote/RSPG/profile/> .
...
This section looks at rotation across the sectors.

#### Technology to Staples (XLK to XLP)

Since about 2023 these two sectors have had a strong inverse correlation.

XLK stands at
    `r paste("$",sprintf("%3.2f", tail(df.data$XLK.Close,1)),sep="")`
    (last updated on `r index(tail(XLK,1))`). 
    
The 50-day MVA stands at `r paste("$",sprintf("%3.2f", tail(df.data$XLK.Close__mva050,1)), sep="")`. 
    
The 200-day MVA stands at `r paste("$",sprintf("%3.2f", tail(df.data$XLK.Close__mva200,1)), sep="")`

XLP stands at
    `r paste("$",sprintf("%3.2f", tail(df.data$XLP.Close,1)),sep="")`
    (last updated on `r index(tail(XLP,1))`). 
    
The 50-day MVA stands at `r paste("$",sprintf("%3.2f", tail(df.data$XLP.Close__mva050,1)), sep="")`. 
    
The 200-day MVA stands at `r paste("$",sprintf("%3.2f", tail(df.data$XLP.Close__mva200,1)), sep="")`

Suggested Citations:

“Technology Select Sector SPDR Fund (XLK) Company Profile & Facts.” Yahoo
Finance, <https://finance.yahoo.com/quote/XLE/profile/> . Accessed 18
Nov. 2025.

“Consumer Staples Select Sector SPDR Fund (XLP) Company Profile & Facts.” Yahoo
Finance, <https://finance.yahoo.com/quote/XLE/profile/> . Accessed 18
Nov. 2025.

```{r eq_rot_xlp_xlk, echo=FALSE, fig.width = 10, fig.asp = .62 }

lst_sym <- c( "X_GSPC.GSPC.Close__YoY", "XLP.Close__YoY", "XLK.Close__YoY")
...
if (require_columns(df.data, lst_sym)) {
  ...
  my.plot <-
    plotSingle(
      ...
      "Date",
      getPlotYLabel(df.symbols, lst_sym[[1]]),
      c(as.Date(Sys.Date()) - 365, Sys.Date()),
      ylim =  c(-25, 50),
      b.legend = TRUE,
      b.percentile = FALSE,
      b.long.legend = TRUE
...
Normalize the ETF funds by the S&P 500 to get a sense of movement relative to
the large cap index.

```{r eq_rot_xlp_xlk_Relative, echo=FALSE, fig.width = 10, fig.asp = .62 }

lst_sym <- c( "XLP__Close__by__X_GSPC__GSPC__Close",
              "XLK__Close__by__X_GSPC__GSPC__Close")

if (require_columns(df.data, lst_sym)) {

    # Set the relative scaling between the two y-axis
  datay.aux.scale <- 0.50
  ...
  my.plot <- my.plot + geom_line(
    ...
    aes(
      x = .data[["date"]],
      y = (.data[[lst_sym[[2]]]] * datay.aux.scale),
      colour = getPlotTitle(df.symbols, lst_sym[[2]])
    ),
    na.rm = TRUE
  )
  ...
  my.plot <-
    my.plot + scale_y_continuous(
      sec.axis = sec_axis( ~ . * (1 / datay.aux.scale), 
      name = paste(substr(lst_sym[[2]],1,3), 
            ".Close/GSPC (", 
            getPlotYLabel(df.symbols, lst_sym[[1]]),
            ")",
            sep="")
...
Lastly, plot  out the index pricing along with the moving averages

```{r rotation_stocks_xlp, echo=FALSE, fig.width = 10, fig.asp = .62 }

lst_sym <- c( "XLP.Close", "XLP.Close__mva050", "XLP.Close__mva200")
...
if (require_columns(df.data, lst_sym)) {
  ...
  my.plot <-
    plotSingle(
      ...
      getPlotYLabel(df.symbols, lst_sym[[1]]),
      c(as.Date('2024-01-01'), Sys.Date()),
      ylim =  c(70, round_up_to(max(df.data[[lst_sym[[1]]]]),1)),
      b.legend = TRUE,
      b.percentile = FALSE,
      b.long.legend = TRUE
    )
...
```{r rotation_stocks_xlk, echo=FALSE, fig.width = 10, fig.asp = .62 }

lst_sym <- c( "XLK.Close", "XLK.Close__mva050", "XLK.Close__mva200")
...
if (require_columns(df.data, lst_sym)) {
  ...
  my.plot <-
    plotSingle(
      ...
      getPlotYLabel(df.symbols, lst_sym[[1]]),
      c(as.Date('2024-01-01'), Sys.Date()),
      ylim =  c(80, round_up_to(max(df.data[[lst_sym[[1]]]]),1)),
      b.legend = TRUE,
      b.percentile = FALSE,
      b.long.legend = TRUE
    )
...
#### Utilities to Staples (XLU to XLP)

Exploratory work.

XLU stands at
    `r paste("$",sprintf("%3.2f", tail(df.data$XLU.Close,1)),sep="")`
    (last updated on `r index(tail(XLF,1))`). 
    
The 50-day MVA stands at `r paste("$",sprintf("%3.2f", tail(df.data$XLU.Close__mva050,1)), sep="")`. 
    
The 200-day MVA stands at `r paste("$",sprintf("%3.2f", tail(df.data$XLU.Close__mva200,1)), sep="")`
...
“State Street Utilities Select Sector SPDR ETF (XLU).” Yahoo! Finance,
https://finance.yahoo.com/quote/XLU/. Accessed 19 Feb. 2026.

“Consumer Staples Select Sector SPDR Fund (XLP) Company Profile & Facts.” Yahoo
Finance, <https://finance.yahoo.com/quote/XLE/profile/> . Accessed 18
Nov. 2025.

```{r eq_rot_xlp_xlu, echo=FALSE, fig.width = 10, fig.asp = .62 }

lst_sym <- c( "X_GSPC.GSPC.Close__YoY", "XLP.Close__YoY", "XLU.Close__YoY")
...
```{r eq_rot_xlp_xlu_Relative, echo=FALSE, fig.width = 10, fig.asp = .62 }

lst_sym <- c( "XLP__Close__by__X_GSPC__GSPC__Close",
              "XLU__Close__by__X_GSPC__GSPC__Close")

if (require_columns(df.data, lst_sym)) {

    # Set the relative scaling between the two y-axis
  datay.aux.scale <- 1.50
...
```{r rotation_stocks_xlp3, echo=FALSE, fig.width = 10, fig.asp = .62 }
...
```{r rotation_stocks_xlu, echo=FALSE, fig.width = 10, fig.asp = .62 }

lst_sym <- c( "XLU.Close", "XLU.Close__mva050", "XLU.Close__mva200")
...
if (require_columns(df.data, lst_sym)) {
  ...
  my.plot <-
    plotSingle(
      ...
      getPlotYLabel(df.symbols, lst_sym[[1]]),
      c(as.Date('2024-01-01'), Sys.Date()),
      ylim =  c(30, round_up_to(max(df.data[[lst_sym[[1]]]]),1)),
      b.legend = TRUE,
      b.percentile = FALSE,
      b.long.legend = TRUE
    )
...
#### Financial to Staples (XLF to XLP)

Since about 2023 these two sectors have had a strong inverse correlation.

XLF stands at
    `r paste("$",sprintf("%3.2f", tail(df.data$XLF.Close,1)),sep="")`
    (last updated on `r index(tail(XLF,1))`). 
    
The 50-day MVA stands at `r paste("$",sprintf("%3.2f", tail(df.data$XLF.Close__mva050,1)), sep="")`. 
    
The 200-day MVA stands at `r paste("$",sprintf("%3.2f", tail(df.data$XLF.Close__mva200,1)), sep="")`
...
“State Street Financial Select Sector SPDR ETF (XLF).” Yahoo! Finance,
https://finance.yahoo.com/quote/XLF/. Accessed 4 Feb. 2026.

“Consumer Staples Select Sector SPDR Fund (XLP) Company Profile & Facts.” Yahoo
Finance, <https://finance.yahoo.com/quote/XLE/profile/> . Accessed 18
Nov. 2025.

```{r eq_rot_xlp_xlf, echo=FALSE, fig.width = 10, fig.asp = .62 }

lst_sym <- c( "X_GSPC.GSPC.Close__YoY", "XLP.Close__YoY", "XLF.Close__YoY")
...
```{r eq_rot_xlp_xlf_Relative, echo=FALSE, fig.width = 10, fig.asp = .62 }

lst_sym <- c( "XLP__Close__by__X_GSPC__GSPC__Close",
              "XLF__Close__by__X_GSPC__GSPC__Close")
...
```{r rotation_stocks_xlp2, echo=FALSE, fig.width = 10, fig.asp = .62 }
...
```{r rotation_stocks_xlf, echo=FALSE, fig.width = 10, fig.asp = .62 }

lst_sym <- c( "XLF.Close", "XLF.Close__mva050", "XLF.Close__mva200")
...
if (require_columns(df.data, lst_sym)) {
  ...
  my.plot <-
    plotSingle(
      ...
      getPlotYLabel(df.symbols, lst_sym[[1]]),
      c(as.Date('2024-01-01'), Sys.Date()),
      ylim =  c(50, round_up_to(max(df.data[[lst_sym[[1]]]]),1)),
      b.legend = TRUE,
      b.percentile = FALSE,
      b.long.legend = TRUE
    )
...
if ( require_columns(df.data, lst_syms ) ){
  ...
  plotSingleQuick(
    ...
    lst_syms[[1]],
    c(-20, 20),
    seq(Sys.Date(), by = "-1 year", length.out = 2)[2],
    b.percentile = TRUE
  )
}
...
Fed Lost Access to Private Jobs Data Ahead of Government Shutdown
    ...
    earnings for millions of workers. The data, which covers 20% of the nation’s
    private workforce, had been available to the Fed with a roughly one-week
    delay—making it both a timely and comprehensive gauge of job-market conditions.
    ...
    bank’s longstanding use of its weekly payroll data, according to a person
    familiar with the matter.

    It couldn’t be learned what prompted the change. The Fed’s use of ADP data
    wasn’t a new revelation.
...
Payroll growth ≈ breadth of labor demand; sustained deceleration usually
foreshadows softer PCE and revenue growth.

Weakening YoY increases odds of easing; re-acceleration keeps policy
tighter for longer—key for duration, curve, and equity style.
...
This section plots out the labor force participation rates for a few of the 
categories. I track it as an indication of consumer health.

### Trends

This plot shows the trends for the 24-54 and 55+ age categories.

```{r LBRForce.Trend, echo=FALSE, fig.width = 10, fig.asp = .32}
...
if (require_columns(df.data, lst_syms )) {
  ...
  y2.min <- min(df.data[[lst_syms[[2]]]], na.rm = TRUE)
  y2.max <- max(df.data[[lst_syms[[2]]]], na.rm = TRUE)
  
  # Scale factor
  scale_factor <- (y1.max - y1.min) / (y2.max - y2.min)
  ...
  my_plot <-plotSingle(
      ...
      "date",
      datay = lst_syms[[1]],
      "Labor Force Participation Rate YoY Trends",
      "Date",
      getPlotYLabel(df.symbols, lst_syms[[1]]),
      c(as.Date('1980-01-01'), Sys.Date()),
      ylim = c(y1.min, y1.max),
      TRUE,
      b.long.legend = TRUE
    )
  ...
  my_plot <-
    ...
    geom_line(
      ...
      aes(
        x = date,
        y = ( .data[[lst_syms[[2]]]] - y2.min ) * scale_factor + y1.min,
        colour = getPlotTitle(df.symbols, lst_syms[[2]])
      ),
      na.rm = TRUE
    ) +
    scale_y_continuous(
      name = getPlotYLabel(df.symbols, lst_syms[[1]]),
      limits = c(y1.min, y1.max),
      sec.axis = sec_axis(
        ~ (. - y1.min) / scale_factor + y2.min,
...
This plot shows the year-over-year growth for the 24-54 and 55+ age catagories.

```{r LBRForce.YoY, echo=FALSE, fig.width = 10, fig.asp = .32}

lst_syms <- c(
              "LNU01300060.Value__YoY",
              "LNS11324230.Value__YoY"
              )
...
plot_gc_over_ppi_with_rhs <- function(dt.start = as.Date("2000-01-01"),
  ...
  # Add secondary axis — suppress the warning about replacing y scale
  p <- suppressWarnings(
...
GLD is the ticker symbol for SPDR® Gold Shares, an exchange-traded fund (ETF)
designed to track the price of gold. Each share represents a fractional
undivided beneficial interest in the SPDR® Gold Trust, which holds physical gold
bullion.
...
portion of transactions considered “specials” "
...
Board of Governors of the Federal Reserve System (US), Monetary Base: Total
[BOGMBASE], retrieved from FRED, Federal Reserve Bank of St. Louis;
https://fred.stlouisfed.org/series/BOGMBASE, November 5, 2025.

```{r BOGMBASE.trend, echo=FALSE}

lst_syms <- c( "BOGMBASE.Value")
...
```
This plot explores the relatationship between the change in the price of gold
and the monetary base.

The vertical lines highlight key events in the market:
- Aug 24, 2015: Flash crash, Dow drops by 1,100 pints in thefirst five minutes 
  of trading.
- Jan 8, 2016: Dow ends one of the worst weeks ever.

```{r MB.to.Gold, echo=FALSE, fig.width = 10, fig.asp = .62 }


datay <- "BOGMBASE.Value__YoY"
dt.start = as.Date('2000-01-01')
ylim <- c(-50, 100)
p1 <- plotSingleQuick(dfRecession, df.data, datay, ylim, dt.start)
p1 <-
...
titlelabel <- paste(datay, " | ", datax)
ylim <- c(30, 200)
ylabel <- getPlotTitle(df.symbols, datay)
xlim <- c(-20, 5)
xlabel <- getPlotTitle(df.symbols, datax)
bLegend <- FALSE
bFitLinear <- FALSE
dt.start = as.Date('1960-01-01')
b.reverse.y = TRUE
...
clients afloat—and only later turn off the taps. This was confirmed by a
recent report from the Bank for International Settlements. It also found
a tight link between this lending cycle and the “real” economy’s booms
and busts."
...
An OAS index is constructed using each constituent bond‚Äôs OAS,
...
"BEA’s featured measure of corporate profits — profits from current
...
financial flows and adjustments, such as deduction for “bad debt.” Thus,
...
consistency across decades of data that’s valuable for researchers. The
...
of information on the economy’s performance.(1) Some important
...
is whether households are borrowing more from banks—or in other words,
...
funds provides insights into a sector’s health and the performance of
...
set of tables in the Federal Reserve’s Z.1 statistical release.

(1) Teplin, Albert M. “The U.S. Flow of Funds Accounts and Their Uses.”
    Federal Reserve Bulletin, July 2001;
    <http://www.federalreserve.gov/pubs/bulletin/2001/0701lead.pdf>.
(2) Board of Governors of the Federal Reserve System. “Guide to the Flow
    of Funds Accounts.” 2000,
