...
if(bRefresh) {
  ...
  for (idx in 1:nrow(df.symbols)) {
    ...
    if ( grepl("=", df.symbols[idx, "string.symbol"], fixed = TRUE) ) {
      ...
      colnames(CLF) = c("CLF.Open","CLF.High","CLF.Low","CLF.Close","CLF.Volume","CLF.Adjusted")
      ...
      # Projected GDP 
      # if (str.symbol.list == "GDP"){
      #   GDP <- rbind(GDP, xts(x = 19384.15, as.Date("2020-03-31") ))
      # }
...
df.silverblatt <- data.frame(read_excel('sp-500-eps-est.xlsx', range='QUARTERLY DATA!A6:I139'))
...
df.silverblatt.est <-
  data.frame(read_excel('sp-500-eps-est.xlsx', range = 'ESTIMATES&PEs!A127:J134'))
...
```{r predplot_switch.1, echo=FALSE}
...
```{r train.equation.smooth.1, echo=FALSE}
...
```{r predict.smooth.all, echo=FALSE}
...
```{r predict.plot.smooth.1, echo-FALSE}
...
```{r SP500pltNear.similar.TimeLine, echo=FALSE, fig.width = 7, fig.asp = 1.1 }
datay <- "GSPC.Close"
ylim <- c(2000, 4000)
p1 <- plotSingle(dfRecession, df.data, "date", datay, getPlotTitle(df.symbols, datay), "Date", 
            getPlotYLabel(df.symbols, datay), c(Sys.Date()-60, Sys.Date()), ylim, TRUE)


datay <- "GSPC.Close"
ylim <- c(12.5, 18)
dt.Start <- as.Date('1937-07-19')
dt.End <- as.Date('1937-09-17')
p2 <- plotSingle(dfRecession, df.data, "date", datay, getPlotTitle(df.symbols, datay), "Date", 
            getPlotYLabel(df.symbols, datay), c(dt.Start, dt.End), ylim, TRUE)
...
```{r SP500TTMPE.Silver,  echo=FALSE, fig.width = 9, fig.asp = 0.4 }
...
#### DPCERC: Personal consumption expenditures (PCE) - Line ...
...
```{r newhousing.yoy, fig.width = 7, fig.asp = 1.3 }
