...
us_confirmed_long_jhu <- read_csv(jhu_url) %>% rename(province = "Province/State",  country_region = "Country/Region")
...
Correlation across some of the series.
...
# Document the residual as wel
str.symbol.res <- "U6.lm.U3.res"
df.data[str.symbol.res] <- df.data$U6RATE - df.data[str.symbol]

# Add the u6 linear model model prediction residual to the symbols table
df.symbols <-
  rbind(
    df.symbols,
    data.frame(
...
### Employment Correlation, U3 (UNRATE) and ICSA 
...
# I break the data into three sets: 50% for training, 25% for testing, and 25% for validation.
set.seed(123456)
in.train <- createDataPartition(y=df.emp.model$RecInit, p = 0.50, list=FALSE)
df.train <- df.dataModel[in.train,]
df.data.rest <- df.emp.model[-in.train,]
in.val <- createDataPartition(y = df.data.rest$RecInit, p = 0.50, list = FALSE)
...
# Document the residual as well
str.symbol.res.knn <- "U3.knn.ICSA.res"
...
```{r U3.pred.ICSA.plot, echo=FALSE, fig.width = 10, fig.asp = .62}


datay <- str.symbol.knn
datay.aux <- str.symbol
datay.aux.1 <- "UNRATE"
ylim <- c(0, 15)
dt.end <- Sys.Date()
myPlot <-
...
```{r U3.pred.ICSA.plot.recent, echo=FALSE, fig.width = 10, fig.asp = .62}
...
```{r U3.pred.CCSA.plot, echo=FALSE,, fig.width = 10, fig.asp = .62}
...
```{r U3.pred.CCSA.plot.recent, echo=FALSE,, fig.width = 10, fig.asp = .62}
