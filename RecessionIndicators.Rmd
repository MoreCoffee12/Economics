...
#
# Pros: minimal refactor; your existing SYMBOL.FIELD parsing still works. 
#
...
if (!is.null(sym_tbl)) {
  needed <- c("string.symbol", "string.description")
  ...
  desc_map <- setNames(
    ...
    as.character(sym_tbl$string.symbol)
  )
...
- S&P 500 Volume, last updated on `r index(tail(GSPC,1))`, is `r getTrendString(df.data, 'X_GSPC.GSPC.Volume', 365)` over the last year and `r getTrendString(df.data, 'X_GSPC.GSPC.Volume', 30)` over the last month. 
...
datay <- "GSPC__CloseBYMDY__Close__YoY"
ylim <- c(-50, 75)
dtStart = as.Date('1980-01-01')
plotSingle(dfRecession, df.data, "date", datay, getPlotTitle(df.symbols, datay), "Date", 
