...
if (bRefresh) {
  ...
  for (idx in 1:nrow(df.symbols)) {
    # This ticker symbol is an odd one. R must allow '=' as variable names, but
    # most safe names I found removed it. Manually changed here.
    if (grepl("=", df.symbols[idx, "string.symbol"], fixed = TRUE)) {
      CLF <- getSymbols(
      ...
      str.symbol.list <- df.symbols[idx, "string.symbol"]
      
      print(str.symbol.list)
    ...
    str.safe <-
      str_replace_all(df.symbols[idx, "string.symbol"], "[^[:alnum:]]", "")
    assign(str.safe, get(str.safe)[!duplicated (index(get(str.safe))), ])
    
    # Grab the series for some additional extractions
    xts.temp <- get(str.safe)
...
if (bRefresh){
  ...
  # Refiner Motor Gasoline Sales Volumes
    ...
    PETA103600001M <- xts( tlb.eia [,-1], order.by=as.Date(as.yearmon(tlb.eia[['period']])))
    PETA103600001M <- PETA103600001M[,9]
    colnames(PETA103600001M) <- c(str.name)
  ...
  # U.S. Regular Gasoline Retail Sales by Refiners, Monthly
    ...
    # Convert the table to an xts object an and select the column we are interested in
    PETA123600001M <- xts( tlb.eia [,-1], order.by=as.Date(as.yearmon(tlb.eia[['period']])))
    PETA123600001M <- PETA123600001M[,9]
    colnames(PETA123600001M) <- c(str.name)
    ...
    # Convert the table to an xts object an and select the column we are interested in
    PETA143B00001M <- xts( tlb.eia [,-1], order.by=as.Date(as.yearmon(tlb.eia[['period']])))
    PETA143B00001M <- PETA143B00001M[,9]
    colnames(PETA143B00001M) <- c(str.name)
    ...
    # Convert the table to an xts object an and select the column we are interested in
    PETA133B00001M <- xts( tlb.eia [,-1], order.by=as.Date(as.yearmon(tlb.eia[['period']])))
    PETA133B00001M <- PETA133B00001M[,9]
    colnames(PETA133B00001M) <- c(str.name)    
    ...
    # Convert the table to an xts object an and select the column we are interested in
    TOTALOGNRPUSM <- xts( tlb.eia [,-1], order.by=as.Date(as.yearmon(tlb.eia[['period']])))
    TOTALOGNRPUSM <- TOTALOGNRPUSM[,3]
    colnames(TOTALOGNRPUSM) <- c(str.name)    
    ...
    # Convert the table to an xts object an and select the column we are interested in
    TOTALPANRPUSM <- xts( tlb.eia [,-1], order.by=as.Date(as.yearmon(tlb.eia[['period']])))
    TOTALPANRPUSM <- TOTALPANRPUSM[,3]
    colnames(TOTALPANRPUSM) <- c(str.name)    
    ...
    # Convert the table to an xts object an and select the column we are interested in
    TOTALNGNRPUSM <- xts( tlb.eia [,-1], order.by=as.Date(as.yearmon(tlb.eia[['period']])))
    TOTALNGNRPUSM <- TOTALNGNRPUSM[,3]
    colnames(TOTALNGNRPUSM) <- c(str.name)    
...
if(bRefresh){
  ...
  colnames(BKRTotal)[[1]] <- c(str.safe.name)
  ...
  colnames(BKRGas)[[1]] <- c(str.safe.name)
  ...
  colnames(BKROil)[[1]] <- c(str.safe.name)
...
if(bRefresh){
  ...
  # Read in th Excel file, starting with the quaterly data
  df.silverblatt <- data.frame(read_excel('sp-500-eps-est.xlsx', range='QUARTERLY DATA!A7:I143'))
...
```{r covid.hell.tidy, echo=FALSE}
if(bRefresh){
  
  df.corona.region <- coronavirus %>%
  ...
  df.corona.date <- coronavirus %>%
  filter(type == "confirmed") %>%
  group_by(country, date) %>%
  summarise(total = sum(cases)) %>%
  arrange(country) 
  
  df.corona.date.us <- df.corona.date[df.corona.date$country=='US',]
  df.corona.date.us$day <- as.factor(weekdays(df.corona.date.us$date))
  df.corona.date.us$day_number <- recode(df.corona.date.us$day, 
         "Sunday" = "0",
         "Monday" = "1",
         "Tuesday" = "2",
         "Wednesday" = "3",
         ...
         "Friday" = "5",
         "Saturday"= "6")
  df.corona.date.us$day_number <- as.numeric(as.character((df.corona.date.us$day_number)))
}
...
```{r covid.hell.plot, echo=FALSE}
qplot(df.corona.date.us$date, df.corona.date.us$total)
...
my.plot <-
  qplot(day_number,
        total,
        data = df.corona.date.us[1:4, ],
        geom = c("point", "line"))
i.lines <- floor(nrow(df.corona.date.us) / 7) - 3
idx.start <- 5
for (idx in 1:i.lines) {
  idx.end = idx.start + 6
  my.plot <-
    my.plot + geom_line(data = df.corona.date.us[idx.start:idx.end, ], aes(x = day_number, y = total))
  idx.start = idx.end + 1
}
idx.end = idx.start + 6
my.plot <-
  my.plot + geom_line(
    data = df.corona.date.us[idx.start:idx.end,],
    aes(x = day_number, y = total),
    color = "green",
    size = 2
  )
idx.start <- idx.end + 1
...
my.plot <-
  my.plot + ggtitle("US Daily Cases, JHU Data (Red is most recent, Blue is last week,\nGreen the week before that )")
my.plot <- my.plot + xlab("Day Number, 0=Sunday, 6 = Saturday")
my.plot <- my.plot + ylab("Daily Total")
my.plot
...
# my.plot<- df.data %>% plot_ly(x = ~index(df.data), type="candlestick",
#           open = ~CLF.Open, close = ~CLF.Close,
#           high = ~CLF.High, low = ~CLF.Low) 
# my.plot <- my.plot %>% layout(title = "Basic Candlestick Chart")
# 
# my.plot
...
datay <- "CLF.Volume_YoY"
ylim <- c(-150, 150)
p3 <- plotSingleQuick(dfRecession, df.data, datay, ylim, dtStart)
...
datay.aux.1 <- "CLF.Close"
dtStart = as.Date('2019-01-01')
my.plot <-
  plotSingleQuick(
    dfRecession,
    df.data,
...
datay <- "DCOILWTICO.by.PPIACO"
datay.aux <- "DCOILBRENTEU.by.PPIACO"
dtStart = as.Date('1987-01-01')
my.plot <-
  plotSingleQuick(
    dfRecession,
...
b.legend <- TRUE
b.percentile <- TRUE
plotSingleQuick(dfRecession, df.data, datay, ylim, dtStart, b.legend, b.percentile)
...
## Ticker price change
datay <- "BKR.Close_YoY"
datay.aux.1 <- "SLB.Close_YoY"
datay.aux.2 <- "HAL.Close_YoY"
p2 <-
  plotSingleQuick(
    dfRecession,
    df.data,
...
datay <- "TOTLLNSA"
datay.aux <- "DPSACBW027SBOG"
datay.title <- getPlotTitle(df.symbols, datay.aux, str.sep = "\n")
datay.aux.1 <- "WRESBAL"
datay.title.1 <-
  getPlotTitle(df.symbols, datay.aux.1, str.sep = "\n")
datay.aux.2 <- "TOTLLNSA.PLUS.WRESBAL"
datay.title.2 <-
...
datay.aux.3 <- "MSPUS.times.HSN1FNSA.plusEXHOSLUSM495S"
ylim <- c(1000, 3000)
...
datay.aux.3 <- "MSPUS.times.HSN1FNSA.plusEXHOSLUSM495S.by.GDP"
ylim <- c(5, 16)
dt.start <- as.Date("1jun2004","%d%b%Y")
my.plot <- plotSingle(
  dfRecession,
...
datay<- "CONSUMERNSA.INTEREST_YoY"
ylim <- c(-15, 30)
b.legend <- FALSE
b.percentile <- TRUE
p2 <- plotSingle(dfRecession, df.data, "date", datay, getPlotTitle(df.symbols, datay), "Date", 
...
datay <- "HOUST_YoY"
datay_aux <- "HOUSTNSA_YoY"
ylim <- c(-50, 100)
dtStart = as.Date('1960-01-01')
b.percentile <- TRUE
my.plot <- plotSingleQuick(dfRecession, df.data, datay, ylim, dtStart, b.percentile)
