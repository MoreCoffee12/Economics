...
### Employment Correlation, U3 (UNRATE) and ICSA_YoY with PAYNSA_YoY

Calculate linear model fit across the series.

```{r U3.pred, echo=FALSE}
...
# Add prediction to the dataframe
str.symbol <- "U3.lm.ICSA_YoY.PAYNSA_YoY"
df.data[str.symbol] <-
...
df.symbols <-
  rbind(
    ...
    data.frame(
      string.symbol = str.symbol,
      string.source = "Predict",
      string.description = "Linear Model Prediction\nU3 given ICSA_YoY and PAYNSA_YOY",
      string.label.y = "(-)",
      float.expense.ratio = -1.00,
      Max030 = FALSE,
      Max180 = FALSE,
...
# Document the residual as wel
str.symbol.res <- "U3.lm.ICSA_YoY.PAYNSA_YoY.res"
...
Plot the measured U3 (UNRATE) to U6 predicted by a linear fit to ICSA and PAYNSA_YoY.

```{r U3.pred.plot, echo=FALSE,, fig.width = 10, fig.asp = .62}


datay <- "UNRATE"
datay_aux <- str.symbol
ylim <- c(0, 15)
dt.end <- as.Date("2020-05-01")
myPlot <-
  plotSingle(
    dfRecession,
    df.data,
