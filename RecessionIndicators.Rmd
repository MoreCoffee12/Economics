...
if(bRefresh) {
  ...
  df.symbols <-
    rbind(
      ...
      data.frame(
        string.symbol = str.symbol.raw,
        string.source = "QUANDL",
        string.description =  "Gold Price: London Fixings, London Bullion Market Association (LBMA). ",
  ...
  # Create a new xts object with the selected column
  assign(str.name, xts(df.temp[, -1], order.by = df.temp[, 1]))
...
```{r realperincYoY, echo=FALSE, fig.width = 5, fig.asp = .42}
datay <- "RPI_YoY"
ylim <- c(-10, 15)
