...
my.plot <- qplot(day_number, total, data=df.corona.date.us[1:4,], geom=c("point", "line"))
i.lines <- floor(nrow(df.corona.date.us)/7)-2
idx.start <- 5
for (idx in 1:i.lines){
  idx.end = idx.start+6
  my.plot <- my.plot + geom_line(data = df.corona.date.us[idx.start:idx.end,], aes(x = day_number, y = total)) 
  idx.start = idx.end + 1
}
idx.end = idx.start+6
my.plot <-
  my.plot + geom_line(data = df.corona.date.us[idx.start:idx.end, ], aes(x = day_number, y = total), color="red") 

my.plot <- my.plot + ggtitle("US Daily Cases, JHU Data (Red is most recent)")
