...
my_plot <-
  plotSingle(
    ...
    "Date",
    getPlotYLabel(df.symbols, str_symbol),
...
#lst_syms <- c("UNRATE.Value", "U6RATE.Value")
...
str_symbol <- "CCSA.by.CLF16OV__lm__POPTHM"
df.data[str_symbol] <- df.data$CCSA / df.data$CLF16OV__lm__POPTHM
...
datay <- "CCSA.by.CLF16OV__lm__POPTHM"
datay.aux <- "UNRATE"
ylim <- c(0, 50)
dt.end <- Sys.Date()
my_plot <-
...
# Add linear prediction to the dataframe
str_symbol <- "NPPTTL.by.CLF16OV__lm__POPTHM"
df.data[str_symbol] <- 100 * ( 1 - (df.data$NPPTTL / df.data$CLF16OV__lm__POPTHM) )
...
datay <- "NPPTTL.by.CLF16OV__lm__POPTHM"
datay.aux <- "UNRATE"
ylim <- c(0, 40)
dt.end <- Sys.Date()
...
str_symbol.res <- "U6.lm.U3.res"
df.data[str_symbol.res] <- df.data$U6RATE - df.data[str_symbol]
...
# Document the residual as well
str_symbol.res.knn <- "U6.knn.U3.res"
df.data[str_symbol.res.knn] <- df.data$U6RATE - df.data[str_symbol.knn]
...
# str_symbol.nnet <- "U6.nnet.U3"
# df.data[str_symbol.nnet] <-
...
# 
# # Document the residual as well
# str_symbol.res.nnet <- "U6.nnet.U3.res"
# df.data[str_symbol.res.nnet] <- df.data$U6RATE - df.data[str_symbol.nnet]
# 
...
#     df.symbols,
#     data.frame(
#       string.symbol = str_symbol.res.nnet,
#       string.source = "Predict Resid.",
...
# hist(d.test.resid.nnet, main=paste("Residual For", str_symbol.res.nnet, sep=" "))
...
list.fit.u3icsapaynsa <- train(
  UNRATE ~ ICSA.by.CLF16OV__lm__POPTHM,
  data = df.train,
  method = "lm",
  preProcess = c('center', 'scale')
)
...
# Add linear prediction to the dataframe
str_symbol <- "U3.lm.ICSA.by.CLF16OV__lm__POPTHM"
df.data[str_symbol] <-
...
# Document the residual as well
str_symbol.res <- "U3.lm.ICSA.by.CLF16OV__lm__POPTHM.res"
...
# Add knn prediction to the dataframe
str_symbol.knn <- "U3.knn.ICSA.by.CLF16OV__lm__POPTHM"
df.data[str_symbol.knn] <-
...
# Document the residual as well
str_symbol.res.knn <- "U3.knn.ICSA.by.CLF16OV__lm__POPTHM.res"
df.data[str_symbol.res.knn] <- df.data$UNRATE - df.data[str_symbol.knn]
